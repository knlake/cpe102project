...
def next_position(world, entity_pt, dest_pt): #keep as function
...
#def blob_to_vein(world, entity, vein): #blob trying to move to vein. nearest vein place in blob
       #entity_pt = entity.get_position()
       #if not vein: #no veins on board
          #return ([entity_pt], False)
       #vein_pt = vein.get_position()
       #if adjacent(entity_pt, vein_pt): #next to vein
           #remove_entity(world, vein)
           #return ([vein_pt], True)
       #else:
          #new_pt = blob_next_position(world, entity_pt, vein_pt) #only passing blob's position, place blob_next_position leave as function because works on points and world or miner because function of miner
...
def remove_entity(entity, world): #world class or keep out
...
#def schedule_ore(ore, world, ticks, i_store):
...
#def schedule_vein(vein, world ticks, i_store):
...
def schedule_animation(entity, world, repeat_count=0):
   schedule_action(world, entity,
      create_animation_action(world, entity, repeat_count),
      entity.get_animation_rate())


def clear_pending_actions(world, entity): #goes in world
   for action in entity.get_pending_actions():
      world.unschedule_action(action)
   entity.clear_pending_actions()
...
#blob next_positions
#is_occupied out of world class



#def miner_to_ore(world, entity, ore):  #minernot full class
   #entity_pt = entity.get_position()
...
#def miner_to_smith(world, entity, smith): #is the miner next to smith miner full class
   #entity_pt = entity.get_position()
...
#def create_miner_not_full_action(world, entity, i_store): #minernotfull's action
   #def action(current_ticks):
      #entity.remove_pending_action(action)

      #entity_pt = entity.get_position()
      #ore = world.find_nearest(entity_pt, entities.Ore) #find nearest ore
      #(tiles, found) = miner_to_ore(world, entity, ore) #move to ore
...
#def try_transform_miner_not_full(world, entity):
   #if entity.resource_count < entity.resource_limit: #carrying fewer than capacity. can only hold 2
      #return entity
   #else:
...
#def try_transform_miner(world, entity, transform): #call function check to see if changed.
   #new_entity = transform(world, entity)
   #if entity != new_entity: #did change?
      #clear_pending_actions(world, entity)
      #world.remove_entity_at(entity.get_position())
      #world.add_entity(new_entity)
      #schedule_animation(world, new_entity) #updates world

   #return new_entity
#stay as function or both classes


#def create_miner_action(world, entity, image_store): #into minerfull and minernotfull as 2 separate methods
...
#def create_ore_transform_action(world, entity, i_store): #creates blob if ore not collected. place in ore
   #def action(current_ticks):
      #entity.remove_pending_action(action)
      #blob = create_blob(world, entity.get_name() + " -- blob", #calling create_blob. no blob object place in ore
         #entity.get_position(),
